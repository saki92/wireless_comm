...
for p = 1:length(PtL) %for each SNR values
    Pt = PtL(p);
    error_var = Pt^-alpha; %error gradiant
    %%%%%%%%%%% Channel %%%%%%%%%
    H = Hcap*sqrt(1- error_var) + Htil*sqrt(error_var); %actual
    HVal = Hcap*sqrt(1- error_var) + HtilVal*sqrt(error_var);
    %%%%%%%%%%% Channel %%%%%%%%%
...
%                     U(m,k) = inv( T(m,k) \ Ik); %MMSE weights
        ...
        for m = 1:MV            
            for k = 1:K
                ...
                for i = 1:K
                    if i ~= k
                        Ik = Ik + abs( HVal(:,k,m,ite)' * P(:,i) )^2 + sigma; %Interfearence power
                    end
                end
                gamma(k) = abs( HVal(:,k,m,ite)' * P(:,k) )^2 / Ik;
